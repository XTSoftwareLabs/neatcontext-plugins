...
import { bindKimiSessionId } from "./session.mjs";
...
function takeSessionId(argv) {
  const args = [...argv];
  const index = args.findIndex(
    (value) => value === "--session-id" || value.startsWith("--session-id=")
  );
  if (index === -1) {
    throw new TypeError(
      "Kimi Code session id is required. Run this command through a bundled NeatContext skill."
    );
  }
  ...
  if (token === "--session-id") {
    ...
    value = token.slice("--session-id=".length);
    args.splice(index, 1);
  }
  bindKimiSessionId(value);
  return args;
}
  ...
  try {
    if (typeof capture.targetId === "string" && capture.targetId.length > 0) {
      ...
      const result = await updateCapturedContext({
        ...capture,
        updatedFrom: "kimi-code-conversation"
      });
    ...
    const result = await createCapturedContext({
      ...capture,
      capturedFrom: "kimi-code-conversation"
    });
    await putCard(result.record.id, {
      useWhen: result.routingDescription,
      source: result.profileText
...
async function run() {
  const [command = "status", ...rest] = takeSessionId(process.argv.slice(2));
